...
class QueryService {
    ...
    func postCategory(category: Category, success: @escaping SuccessCreatedCategory, onError: @escaping ErrorResult) {
        
        dataTask?.cancel()
        
        guard let url = URL(string: getCategories) else { return }
        ...
        request.httpBody = try! JSONSerialization.data(withJSONObject: category, options: [])
