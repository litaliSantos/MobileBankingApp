...
extension UIViewController {
    ...
    /// Pushs a ViewController to the current NavigationBar stack
    func push(viewController: UIViewController) {
        self.navigationController?.pushViewController(viewController, animated: true)
    }
    
    //MARK: - NavigationBar
    /// Apply custom changes to navigationBar
    func setCustomNavigationBar() {
        setCustomBackButton()
        setNavigationBarTranslucent()
    }
    
    /// Removes the text from back button and change its color
    private func setCustomBackButton() {
        let backItem = UIBarButtonItem(title: "", style: .plain, target: nil, action: nil)
        navigationItem.backBarButtonItem = backItem
        navigationItem.backBarButtonItem?.tintColor = UIColor.appColor(.darkGreenColor)
    }
    
    /// Makes the NavigationBar transparent
    private func setNavigationBarTranslucent() {
        navigationController?.navigationBar.shadowImage = UIImage()
        navigationController?.navigationBar.isTranslucent = true
        navigationController?.navigationBar.backgroundColor = .clear
        navigationController?.navigationBar.setBackgroundImage(UIImage(), for: .default)
        let textAttributes = [NSAttributedString.Key.foregroundColor: UIColor.appColor(.whiteColor)]
